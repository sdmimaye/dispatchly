...
public sealed class SqlServerMessageOutboxTests
{
    private const string Table = "placed_orders";

    private readonly SqlServerFixture _fixture;

    public SqlServerMessageOutboxTests(SqlServerFixture fixture) => _fixture = fixture;
    ...
    [Fact]
    public async Task EnlistAsync_RejectsANonSqlTransaction()
    {
        var schema = NewSchema();
        using var host = await StartAsync(schema, new Inbox());
        var exception = await Assert.ThrowsAsync<ArgumentException>(() =>
            host.Services.GetRequiredService<IMessageOutbox>()
                ...
                .AsTask());
        Assert.Contains("SqlTransaction", exception.Message, StringComparison.Ordinal);
    }
