...
public sealed class SqlServerTransportTests
{
    private readonly SqlServerFixture _fixture;

    public SqlServerTransportTests(SqlServerFixture fixture) => _fixture = fixture;
    ...
    public async Task PublishAsync_CommittedMessageIsDeliveredAndAcknowledged()
    {
        ...
        await WaitUntilAsync(async () => await CountAsync(schema, "probe_message", "delivered_at IS NOT NULL") == 1);
    }
    ...
    public async Task ExhaustedDelivery_MovesTheRowToTheDeadLetterTable()
    {
        ...
        await WaitUntilAsync(async () => await CountAsync(schema, "probe_message_dead_letter", "1 = 1") == 1);
        Assert.Equal(0, await CountAsync(schema, "probe_message", "1 = 1"));
    }
    ...
    public async Task ExpiredLock_CanBeClaimedAgain()
    {
        ...
        await WaitUntilAsync(() => probe.Attempts.Count >= 1);
        await ExecuteAsync(schema, "UPDATE {0}.probe_message SET locked_until = DATEADD(minute, -1, SYSUTCDATETIME()) WHERE delivered_at IS NULL");
        await host.Services.GetRequiredService<ISqlServerMaintenance>().RedeliverAsync();
        await WaitUntilAsync(() => probe.Attempts.Count >= 2);
    ...
    [Fact]
    public async Task ScheduleRedelivery_RequiresSqlServerAgent()
    {
        var probe = new DeliveryProbe();
        var schema = NewSchema();
        try
        {
            using var host = await StartAsync(schema, probe, options => options.ScheduleRedelivery = true);
            Assert.Equal(1, await CountJobsAsync(schema));
        }
        catch (InvalidOperationException exception)
        {
            Assert.Contains("SQL Server Agent", exception.Message, StringComparison.OrdinalIgnoreCase);
        }
    }

    [Fact]
    public async Task StartAsync_WithoutServiceBrokerThrows()
    {
        var database = "b" + Guid.NewGuid().ToString("N")[..12];
        await _fixture.CreateDatabaseAsync(database);
        var probe = new DeliveryProbe();
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => StartAsync(NewSchema(), probe, options =>
        {
            options.ConnectionString = _fixture.ConnectionStringFor(database);
        }));
        Assert.Contains("Service Broker", exception.Message, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void UseSqlServerTransport_RejectsAGeneralCronExpression()
    {
        var services = new ServiceCollection();
        var exception = Assert.Throws<ArgumentException>(() => services.AddDispatchly().UseSqlServerTransport(options =>
        {
    ...
    private async Task<int> CountJobsAsync(string name)
    {
        await using var connection = new SqlConnection(_fixture.ConnectionString);
        await connection.OpenAsync();
        await using var command = new SqlCommand(
            "SELECT COUNT(*) FROM msdb.dbo.sysjobs WHERE name = @name",
            connection);
