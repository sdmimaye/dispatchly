...
public sealed class SqlServerOutboxEndToEndTests
{
    private const string Table = "placed_orders";

    private readonly SqlServerFixture _fixture;

    public SqlServerOutboxEndToEndTests(SqlServerFixture fixture) => _fixture = fixture;
    ...
    private Task<StoredMessage?> ReadOutboxAsync(string schema, string table, Guid id) =>
        ReadMessageAsync(
            ...
            SELECT JSON_VALUE(payload, '$.orderId'),
                   JSON_VALUE(payload, '$.quantity'),
                   attempt_count,
                   CAST(CASE WHEN delivered_at IS NOT NULL THEN 1 ELSE 0 END AS bit),
                   last_error,
                   CAST(CASE WHEN locked_until IS NULL OR locked_until <= SYSUTCDATETIME() THEN 1 ELSE 0 END AS bit)
            """);
