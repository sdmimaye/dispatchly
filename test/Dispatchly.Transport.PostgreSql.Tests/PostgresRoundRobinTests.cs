...
public sealed class PostgresRoundRobinTests
{
    private const string Table = "placed_orders";

    private readonly PostgresFixture _fixture;

    public PostgresRoundRobinTests(PostgresFixture fixture) => _fixture = fixture;
    ...
    public async Task ExpiredHeartbeat_RedeliveryWakesAnotherHost()
    {
        ...
        await claimed.Services.GetRequiredService<IPostgresMaintenance>().RedeliverAsync();
    ...
    private static void Abandon(IHost host) =>
        host.Services.GetServices<IHostedService>().OfType<PostgresListenService>().Single().Abandon();
    ...
    private async Task<int> CountConsumersAsync(string schema)
    {
        ...
        await using var command = new NpgsqlCommand(
            $"SELECT COUNT(*) FROM {IdentifierRules.Quote(schema)}.{IdentifierRules.Quote(IdentifierRules.ConsumerTable)}",
            connection);
